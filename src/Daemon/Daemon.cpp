...
void renameDataDir() {
  std::string bitcoin2Dir = Tools::getDefaultDataDirectory();
  boost::filesystem::path bitcoin2DirPath(bitcoin2Dir);
  if (boost::filesystem::exists(bitcoin2DirPath)) {
    return;
  }

  std::string dataDirPrefix = bitcoin2Dir.substr(0, bitcoin2Dir.size() + 1 - sizeof(CRYPTONOTE_NAME));

  boost::filesystem::path darkNoteDirPath(dataDirPrefix + "darknote");
  if (boost::filesystem::exists(darkNoteDirPath)) {
    boost::filesystem::rename(darkNoteDirPath, bitcoin2DirPath);
  } else {
    ...
    if (boost::filesystem::exists(boost::filesystem::path(duckNoteDirPath))) {
      boost::filesystem::rename(duckNoteDirPath, bitcoin2DirPath);
    }
  }
}
